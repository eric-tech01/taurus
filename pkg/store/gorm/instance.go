...
import (
	...
	"github.com/eric-tech01/taurus/pkg/utils/xretry"
...
func dial(name string, config *Config) *gorm.DB {
	if config.DSN == "" {
		log.Panic("empty dsn", name)
	}

	dsn, err := parseDSN(config.DSN)
	if err != nil {
		log.Panic("parse dsn", name, err)
	}
	...
	if err != nil {
		if config.OnDialError == "panic" {
			log.Panic("dial mysql db", err, dsn.Addr, name)
		} else {
			log.Panic("dial mysql db", dsn.Addr, err, name)
		}
	}
